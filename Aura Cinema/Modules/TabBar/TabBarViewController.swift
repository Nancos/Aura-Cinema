...
private extension TabBarViewController {
    ...
    func setupLikeButton() {
        likeButton = setupButton(imageName: "heart", title: "Saved", tag: 1, opacity: 0.3)
        likeButton.setTitleColor(UIColor(named: "forLabel"), for: .normal)
        
        customBar.addArrangedSubview(likeButton)
    }
    
    func setupSearchButton() {
        searchButton = setupButton(imageName: "magnifyingglass", title: "Search", tag: 2, opacity: 0.3)
        
        customBar.addArrangedSubview(searchButton)
    }
    
    func setupHomeButton() {
        homeButton = setupButton(imageName: "house", title: "Home", tag: 0, opacity: 1)
        
        customBar.addArrangedSubview(homeButton)
    }
    
    func setupDirectoryButton() {
        directoryButton = setupButton(imageName: "tray", title: "Directory", tag: 3, opacity: 0.3)
        
        customBar.addArrangedSubview(directoryButton)
    }
    
    func setupSettingsButton() {
        settingsButton = setupButton(imageName: "gearshape", title: "Settings", tag: 4, opacity: 0.3)
