...
protocol FavoritesUpdateDelegate {
...
extension SavedFilmsService {
    func handleError(error: LoadFilmsError) -> (title: String, message: String){
        switch error {
        case .invalidUser:
            return ("Ошибка", "Неверный пользователь")
        case .dataEmpty:
            return ("Ошибка", "Пустые данные")
        case .unknownError:
            return ("Ошибка", "Неизвестная ошибка")
        case .duplicateRemoved:
            return ("", "Фильм удален из избранных!")
        case .firestoreError(let firestoreError):
            return ("Ошибка", "Ошибка Firestore: \(firestoreError.localizedDescription)")
        }
    }
}
